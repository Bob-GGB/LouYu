            ...
            if (view && Once) {
                switch (type) {
                    ...
                    case KHUDWithText:
                    {
                        ...
                        hud.label.text = text;
                        [hud hideAnimated:YES afterDelay:time ? time : 999];
                    }
                        break;
                    default:
                        break;
...
+ (void)ShowWithText:(NSString *)text WithAddToView:(UIView *)view
{
    ...
    hud.mode = MBProgressHUDModeCustomView;//自定义动画类型
    hud.label.text = text;
...
+ (void)ShowTextOnlyWithWindowCenter:(NSString *)text AndShowTime:(NSInteger)time
{
    ...
    hud.alpha = 0.8;
    hud.backgroundColor = [UIColor colorWithRed:0.2 green:0.2 blue:0.2 alpha:1];
//    hud.minShowTime = 0.2;
    hud.margin = 10;
