...
@interface RelateToMeViewController ()<UITableViewDelegate,UITableViewDataSource>
...
@implementation RelateToMeViewController
...
- (void)viewDidLoad {
    ...
    [self getToken];
    [self PostDataToSeverWithUrl:@"http://louyu.qianchengwl.cn/minapp/article/getRelatedForMe"];
}
        ...
        //
            ...
            if ([dict[@"code"] isEqualToNumber:@(-3)]) {
                ...
                for (NSDictionary *obj in arr) {
                    RelatedForMe_NewsListModel *newSModel=[[RelatedForMe_NewsListModel alloc] initWithDict:obj];
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    static NSString *RelateListID=@"RelateListID";
    NewTableViewCell *cell=[tableView dequeueReusableCellWithIdentifier:RelateListID];
    //     NewTableViewCell *cell = [tableView cellForRowAtIndexPath:indexPath];
    if (cell==nil) {
        cell=[[NewTableViewCell alloc] initWithStyle:UITableViewCellStyleSubtitle reuseIdentifier:RelateListID];
    }
    
    
    //    先取到对象
    RelatedForMe_NewsListModel *model=self.newsArr[indexPath.row];
    [cell CallDataWithModel:model];
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    RelatedForMe_NewsListModel *model=self.newsArr[indexPath.row];
    RelateToMe_ConmentsListViewController *RelateToMeCV=[[RelateToMe_ConmentsListViewController alloc] init];
    RelateToMeCV.articleID=model.articleID;
    RelateToMeCV.hidesBottomBarWhenPushed=YES;
    [self.navigationController pushViewController:RelateToMeCV animated:NO];
