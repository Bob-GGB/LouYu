...
@implementation XunJianTableViewCell
...
-(void)moreButtonDidPress:(UIButton*)sender{
    ...
    ExceptionSettleViewController *exceptionVC=[[ExceptionSettleViewController alloc] init];
    if ([self.roleIDnum isEqualToNumber:@1]) {
        if (self.sendController) {
            self.sendController(exceptionVC);
            NSLog(@"我被点击了");
        }
    }
    else if ([self.roleIDnum isEqualToNumber:@3]||[self.roleIDnum isEqualToNumber:@2]) {
        if (self.sendController) {
            approveVC.roleIDNum=self.roleIDnum;
            self.sendController(approveVC);
            NSLog(@"我被点击了");
        }
    }
