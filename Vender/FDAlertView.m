...
- (void)initTitleAndIcon {
    ...
    if (_icon != nil) {
        ...
        _iconImageView.frame = CGRectMake(110, 10, 80, 80);
    ...
    _titleView.frame = CGRectMake(0, 0, _iconImageView.frame.size.width + SPACE_SMALL + titleSize.width,30);
...
- (void)initMessage {
    if (_message != nil) {
        ...
        _messageLabel.frame = CGRectMake(MARGIN_LEFT_LARGE, CGRectGetMaxY(_iconImageView.frame)+20, MAX(contentViewWidth - MARGIN_LEFT_LARGE - MARGIN_RIGHT_LARGE, messageSize.width), messageSize.height);
    ...
    if (_buttonTitleArray.count > 0) {
       ...
       // [_contentView addSubview:horizonSperatorView];
        ...
        for (NSString *buttonTitle in _buttonTitleArray) {
            ...
            _button = [[UIButton alloc] initWithFrame:CGRectMake(index * buttonWidth+30, horizonSperatorView.frame.origin.y + horizonSperatorView.frame.size.height, 100, 44)];
            _button.titleLabel.font = [UIFont systemFontOfSize:BUTTON_FONT_SIZE];
            [_button setBackgroundColor:[UIColor orangeColor]];
            _button.layer.cornerRadius = 5.0;
            _button.layer.masksToBounds = YES;
            [_button setTitle:buttonTitle forState:UIControlStateNormal];
            [_button setTitleColor:[UIColor whiteColor] forState:UIControlStateNormal];
            [_button addTarget:self action:@selector(buttonWithPressed:) forControlEvents:UIControlEventTouchUpInside];
            [_buttonArray addObject:_button];
            [_contentView addSubview:_button];
            
            if (index < _buttonTitleArray.count - 1) {
                UIView *verticalSeperatorView = [[UIView alloc] initWithFrame:CGRectMake(_button.frame.origin.x + _button.frame.size.width, _button.frame.origin.y, 1, _button.frame.size.height)];
